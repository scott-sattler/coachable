...
if __name__ == '__main__':
    ...
    #     solutions = list()
    #
    #     # for each cell, dfs top/left and bottom/right
    #     for i in range(rows):
    #         for j in range(cols):
    #             visited = set()
    #             pac = self.dfs(heights, visited, ((-1, 0), (0, -1)), i, j)
    #             visited = set()
    #             atl = self.dfs(heights, visited, ((1, 0), (0, 1)), i, j)
    #             if pac and atl:
    #                 solutions.append([i, j])
    #
    #     return solutions
    #
    # def dfs(self, heights, visited, directions, i, j):
    #     rows = len(heights)
    #     cols = len(heights[0])
    #
    #     # exclude visited cells
    #     if (i, j) in visited:
    #         return False
    #     visited.add((i, j))
    #
    #     # look if near goal state (also boundary error)
    #     n_i = i + directions[0][0]
    #     n_j = j + directions[1][1]
    #     if n_i < 0 or n_j < 0 or n_i >= rows or n_j >= cols:
    #         return True
    #
    #     # revert step if impermissible
    #     if heights[i][j] < heights[n_i][j]:
    #         n_i = i
    #     if heights[i][j] < heights[i][n_j]:
    #         n_j = j
    #
    #     # calls to current vertex will be
    #     # rejected as a visited node
    #     return self.dfs(heights, visited, directions, n_i, j) or self.dfs(heights, visited, directions, i, n_j)