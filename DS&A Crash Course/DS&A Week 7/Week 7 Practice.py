...
Dynamic Programming and Recurrence Examples
    ...
    Unit Tests. Identify the solution cases up to n= 4. For 2D, do all cases up to (3,3)

    Recurrence Relation.  Identify the recurrence relationship and base cases. Explain why they are true by giving a
    qualitative explanation in plain English.

    Bottom Up. Compute a small example using a bottom-up (n = 6, m = 6)

    Top Down Approach. Compute a small example using a top-down approach with memorization. (n=6,m=6)

    Complexity Analysis. Identify the optimal runtime and space complexity.

    Is DP worth it? Does dynamic programming improve the runtime compared to a recursive approach?

Problems That Can Be Solved with Recursion

    Number of paths up a staircase of length N where you take 1 or 2 steps each time. Example provided.

    Computing the number of permutations of [1-n] i.e. [1,2,3,4,5,...n-1,n]

    Unique Paths: The number of paths from the top left corner of a grid to the bottom right corner when moving only
    down and to the right.

    Given a 2xN grid, how many different ways can you fill the gird with 2x1 dominoes?

    Given a set A = {1,2,3,...,N} calculate the number of possible subsets of A.

    Count the number of functions from {1,2,3,...,N} to a set of size {1,2,3,...,M}. Here is an additional explanation
    of functions.

    Extension of problem 6. A function has a fixed point if f(x) = x for any x in the domain of f . How many functions
    are there from {1,2,3,...,N} to {1,2,3,...,M} without any fixed points?

    Given a 4xN grid, how many different ways can you fill the grid with 2x1 dominoes?
...
OPTIONAL: Binary Tree Recursive Challenge Problems
    ...
    How many binary trees can you create with N distinct integers? Assume N=2^n-1 i.e. so N is the number of elements in
    a complete binary tree.
    
    How many binary search trees can you create with N=2^n-1 distinct integers?
    
    How many distinct binary heaps can you  create with N=2^n-1 distinct integers?
    
    We say a binary tree is level-sorted if every element is larger than every element in a lower level (even if they
    ...
    f(n) = f(n-1) + f(n-2) This is because to get to step n you can get there by taking 1 step from n-1 or 2 steps from
    n-2 . Therefore the number of paths to n is the total number of paths from n-1 plus the number of paths from n-2
    ...
    Runtime is O(n) and space O(1) if we only cache the previous 2 elements.
    
    Yes. If we used plain recursion, the runtime would be exponential. Dynamic programming gets us an O(n) runtime.
